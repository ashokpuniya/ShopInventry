...
import com.ashok.shopInventory.dto.*;
...
import com.ashok.shopInventory.service.*;
...
public class ELearningWebServiceController {
    ...
    public String home(Model model,@PathVariable("userId") Integer userId) {
    ...
    @RequestMapping(path = "/compitation/{course}/{userId}")
    public String showEvents(Model model,@PathVariable("userId") String userId,@PathVariable("course") String course) {
    ...
    @RequestMapping(path = "/showQuiz/{course}/{subject}/{userId}")
    public String createQuiz(Model model,@PathVariable("course") String course,@PathVariable("subject") String subject,@PathVariable("userId") String userId) {
    ...
    @RequestMapping(path = "/startQuiz/{course}/{subject}/{quizNumber}/{userId}")
    public String startQuiz(Model model,@PathVariable("course") String course,@PathVariable("subject") String subject,@PathVariable("quizNumber") String quizNumber,@PathVariable("userId") String userId) {
...
//        }
        ...
        List<QuizResult> quizResults = quizResultService.getResultByCourseAndSubjectAndQuizNumberAndUser(course,subject,quizNumber,Long.parseLong(userId));
    ...
    @RequestMapping(path = "/saveQuiz/{course}/{subject}/{quizNumber}/{userId}")
    public String saveQuiz(@ModelAttribute QuestionParserDto qForm, Model model, @PathVariable("course") String course, @PathVariable("subject") String subject, @PathVariable("quizNumber") String quizNumber,@PathVariable("userId") String userId) {
        List<QuizResult> quizResults = quizResultService.getResultByCourseAndSubjectAndQuizNumberAndUser(course,subject,quizNumber,Long.parseLong(userId));
        if(quizResults.size()>0){
            model.addAttribute("userId",userId);
            model.addAttribute("message","You Have Already Attempt this Test.");
       ...
       int Correct = quizDetailService.saveQuizResult(qForm);
        ...
        model.addAttribute("quizNumber",quizNumber);
        model.addAttribute("userId",userId);
        CalculateQuizResultAndSave(course,subject,quizNumber,Correct,qForm.getQuizData().size(),userId);
       return "SuccessPage";
    }

    @RequestMapping(path = "/showScoreBoard/{course}/{subject}/{quizNumber}/{userId}")
    public String showScoreBoard(Model model,@PathVariable("course") String course,@PathVariable("subject") String subject,@PathVariable("quizNumber") String quizNumber,@PathVariable("userId") String userId) {
        List<ShowResult> showResults = getRankByCourseAndSubjectAndQuizNumber(course,subject,quizNumber,userId);
    ...
    private Map<String, String> getFieldTypeMap(Class clazz) {
        ...
        for (Field field : declaredFields) {
            if (field.isAnnotationPresent(Column.class) || field.isAnnotationPresent(JoinColumn.class)||true) {
    ...
    private void CalculateQuizResultAndSave(String course, String subject, String quizNumber, int correct, int size, String userId) {
        try{
//            String email = WebContextUtils.getCurrentUserEmail();
//            User user = WebContextUtils.getCurrentUser();
...
//            System.out.println(user);
            ...
            quizResult.setUserId(Integer.valueOf(userId));
            quizResultService.save(quizResult);
        }catch (Exception e){
            LOG.info("Exception occour during save Result:{}",e);
        }
    }
    private List<ShowResult> getRankByCourseAndSubjectAndQuizNumber(String course, String subject, String quizNumber, String userId) {
...
//        User user = WebContextUtils.getCurrentUser();
        ...
        for(QuizResult quizResult:quizResults){
            ...
            //todo
            if(quizResult.getUserId()==Long.parseLong(userId)){
                first=showResult;
